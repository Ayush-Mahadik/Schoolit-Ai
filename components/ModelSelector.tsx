  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        className="h-8 px-2.5 text-sm bg-surface-3 hover:bg-surface-4 border border-transparent hover:border-orange-500/30 rounded-lg transition-colors flex items-center gap-1.5 font-medium"
        title="Select AI Model"
      >
        <Icon name={current.icon} className="w-3.5 h-3.5 text-orange-400" />
        <span className="hidden sm:inline text-white text-xs font-bold">{current.name}</span>
        <ChevronDown className={`w-3 h-3 transition-transform text-orange-500/60 ${open ? "rotate-180" : ""}`} />
      </button>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute right-0 top-full mt-1.5 w-64 bg-surface-2 border border-orange-500/20 rounded-xl shadow-2xl z-[100] overflow-hidden"
          >
            <div className="p-1.5">
              <p className="text-[10px] uppercase tracking-wider text-orange-500/50 font-bold px-2.5 py-1.5">
                AI Model
              </p>
              {MODEL_OPTIONS.map((model) => (
                ...
                >
                  <div className={`w-7 h-7 rounded-md flex items-center justify-center shrink-0 ${
                    activeModel === model.id ? "bg-orange-500/20" : "bg-surface-3"
                  }`}>
                    <Icon name={model.icon} className={`w-3.5 h-3.5 ${
                      activeModel === model.id ? "text-orange-400" : "text-slate-400"
                    }`} />
                  </div>
...
function SpeedBadge({ speed }: { speed: "fast" | "medium" | "slow" }) {
  const config = {
    ...
    medium: { label: "Balanced", color: "bg-orange-500/15 text-orange-400" },
